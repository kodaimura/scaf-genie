...
shell:
	$(DOCKER_COMPOSE_RUN) $(API_SERVICE) bash
...
routes:
	$(DOCKER_COMPOSE_RUN) $(API_SERVICE) julia --project=. -e 'using Pkg; Pkg.instantiate(); using Genie; Genie.loadapp(); println("GET /health"); println("POST /api/auth/signup"); println("POST /api/auth/login"); println("POST /api/auth/refresh"); println("POST /api/auth/logout"); println("POST /api/auth/forgot-password"); println("GET /api/auth/reset-password/verify"); println("POST /api/auth/reset-password"); println("GET /api/accounts"); println("POST /api/accounts"); println("GET /api/accounts/me"); println("PUT /api/accounts/me/password"); println("PUT /api/accounts/:target_account_id::Int/disable"); println("PUT /api/accounts/:target_account_id::Int/enable"); println("GET /api/accounts/:target_account_id::Int"); println("PUT /api/accounts/:target_account_id::Int")'

migrate:
	$(DOCKER_COMPOSE_RUN) $(MIGRATE_SERVICE)

current:
	$(DOCKER_COMPOSE_RUN) $(MIGRATE_SERVICE) julia --project=. -e 'using Pkg; Pkg.instantiate(); using Genie; Genie.loadapp(); using SearchLight; SearchLight.Migration.status()'

history:
	$(DOCKER_COMPOSE_RUN) $(MIGRATE_SERVICE) julia --project=. -e 'using Pkg; Pkg.instantiate(); using Genie; Genie.loadapp(); using SearchLight; SearchLight.Migration.status()'

versions:
	$(DOCKER_COMPOSE_RUN) $(API_SERVICE) julia --project=. -e 'using Pkg; Pkg.instantiate(); using Genie; println("Julia ", VERSION); println("Genie ", Pkg.dependencies()[Base.UUID("c43c736e-a2d1-11e8-161f-af95117fbd1e")].version)'
